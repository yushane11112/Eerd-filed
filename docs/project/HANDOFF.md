...
1. 审计旧 `src/game/**` 群岛引擎，决定迁入 legacy、保留迁移工具或删除未用 UI。
2. 继续把服务访问、工人通勤和物流路线汇总成城市热力/拥堵/繁荣可视图层。
3. 将治理卡推荐动作进一步接入建造菜单可用性、阶段锁定和建筑解锁解释。
4. 将承运人货运阶段进一步接入物流拥堵诊断和道路压力指标。
5. 设计 7200 tick/30 日分层长跑方案，验证归档后快照规模长期有界。
