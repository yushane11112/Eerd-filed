...
| 建筑升级 | 无 | 木料 = 下一等级×1；石料 = floor(下一等级/2)×1 | L0–L8 升级施工 |
...
- `buildingUpgradeCost`、`upgradeBuildingFromCityStorage`、`startBuildingUpgradeFromCityStorage` 支持传入自定义表；默认升级曲线保持旧数值。
...
3. 升级成本已进入经济表，但仍只是简单等级倍数，没有按建筑类别、产能、服务容量或文明阶段分层。
...
| 分类型升级曲线 | 不同建筑类别升级投入与收益匹配 | 按 category/type/stage 扩展升级表 |
...
当前完成的是“从硬编码首版规则 → 可导出、可校验、可替换的经济表”，并已覆盖建筑新建、道路和第一版升级成本。

不能把它表述为“经济系统已完成”。真正的商业级经济还必须接入服务容量、排队、仓储吞吐、道路维护、分类型升级曲线和回本周期审计。
