...
- 定向测试：`npm test -- src/simulation/core/SimulationEngine.test.ts src/integration/GameRuntime.test.ts` 通过，2 个测试文件、50 项测试。
- 目标规模 runtime 快照样本：`RENDER_ABLATION_SCENARIO=civilization-scale RENDER_ABLATION_MODES=full RENDER_ABLATION_REPEATS=1 npm run qa:render-ablation` 通过，并隐含完成生产构建。
- 完整测试：`npm test` 通过，58 个测试文件、364 项测试，耗时约 143 秒。已知 jsdom `HTMLCanvasElement.getContext` warning 仍存在但不影响退出码；最慢项仍是多日文明压力场景。
- 最终生产构建：`npm run build` 通过，Vite 构建 2364 个模块。现有大 chunk warning 仍存在，非本轮新增阻断。
- `full`：空白页 rAF 16.64/16.7/16.8ms，游戏页 rAF 149.99/183.3/183.3ms；render sync P95 12.6ms，renderer P95 19.4ms，ticker elapsed P95 183.3ms，scene sync 跳过率 0.727。
- 应用层样本：`runtime.advance` P95/最大 12.3/12.3ms，engine advance P95 12ms，snapshot clone P95 0ms，cache/emit P95 0.3ms；WebGL GPU stall warning 仍出现。
- 结论：runtime snapshot clone 长尾已被消除，但游戏页 rAF 仍明显 RED；下一轮默认目标应继续处理建筑同步长尾、ticker 调度和 WebGL stall。
