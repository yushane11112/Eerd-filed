...
| QA-RENDER-ABLATION-LIFECYCLE-01 | 多次/多环境差分执行器生命周期修复 | `tools/qa/run-render-ablation.ts`, `tools/browser-e2e/run-browser-e2e.cjs` | 进行中 | 本轮重复差分子进程未稳定返回 JSON；需修复端口、服务和浏览器关闭生命周期后再采纳矩阵数据 |
